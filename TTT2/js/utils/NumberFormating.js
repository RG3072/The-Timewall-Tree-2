...
// Basically does the opposite of what normalize in ExpantaNum does
                ...
                if (elem < array.length) {
                    if (height == array[elem][0]) top = array[elem][1] + 1
                        ...
                        if (diff < MAX_LOGP1_REPEATS) {
                            for (let i = 0;i<diff;i++) bottom = Math.log10(bottom) + 1
                        }
                        else bottom = 1 // The increment result is indistinguishable from 1
                        height = array[elem][0]
                        top = array[elem][1] + 1
...
function format(num, precision=2, small=false) {
    ...
    if (num.lt("0.001")) {
      ...
      let mantissa = num.div(ExpantaNum(10).pow(exponent))
